...
int bind_sock(char * port)
{
				...
				for(rp = result; rp != NULL; rp = rp->ai_next)
				{
								...
								{
												printf("socket %s", gai_strerror(errno));
												continue;
								}
								if (setsockopt(sockfd[ret-1], SOL_SOCKET, SO_REUSEPORT, &yes, sizeof(int)) == -1)
								{
												printf("setsockopt %s", gai_strerror(errno));
												close(sockfd[ret-1]);
												sockfd[ret-1] = -1;
												continue;
								}
								...
								{
												printf("bind %s", gai_strerror(errno));
												close(sockfd[ret-1]);
												sockfd[ret-1] = -1;
												continue;
								}
								if(listen(sockfd[ret-1], SOMAXCONN) == -1)
								{
												...
												printf("listen %s", gai_strerror(errno));
												close(sockfd[ret-1]);
												sockfd[ret-1] = -1;
												continue;
								}
								...
								{
												printf("set_nonblocking %s", gai_strerror(errno));
												close(sockfd[ret-1]);
												sockfd[ret-1] = -1;
												continue;
								}
								...
								{
												printf("epoll_ctl %s", gai_strerror(errno));
												close(sockfd[ret-1]);
												sockfd[ret-1] = -1;
												continue;
								}
